...
import {
  generatePageQueryString,
  PageModel,
  PageQuery,
  Tenant,
} from '@vmware/api-interfaces';
import { http } from '@vmware/utils';
...
export function usePrefetchTenants(
  ...
  useEffect(() => {
    if (shouldFetchNextPage) {
      queryClient.prefetchQuery(['getTenants', { ...pageQuery }], () =>
        getTenants(pageQuery)
      );
    }
...
export async function deleteTenant({ id }: { id: string }) {
  return await http.delete(`/core/tenants/${id}`, {
    data: {
      payload:
        'axios DELETE payload needs to wrap with data, unlike POST or PUT',
    },
  });
}
