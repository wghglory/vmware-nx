    ...
    <ul className="flex">
      {steps.map((step, i) => {
        return (
          <li
            key={i}
            className={`flex min-w-[8.75rem] flex-1 flex-col ${
              current === i ? 'active' : ''
            }`}
          >
            <div className="mb-2 whitespace-nowrap text-xs leading-4">
              {step?.header}
            </div>
            {current > i ? (
